...
def batchnorm_var(x, gamma, beta, bn_param):
    mode = bn_param['mode']
...
def layernorm_forward(x, gamma, beta, ln_param):
    ...
    var = np.mean((x - mu) ** 2, axis=1)[:, np.newaxis]
...
def layernorm_backward(dout, cache):
    ...
    dxhat = gamma * dout
    divar = np.sum(dxhat * (x - mu), axis=1)[:, np.newaxis]
    dx1 = dxhat / sq - 1 / D * (x - mu) / (sq ** 3) * divar
    dx2 = -1 / D * np.sum(dx1, axis=1)[:, np.newaxis]
    dx = dx1 + dx2
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def spatial_batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    (N, C, H, W) = dout.shape
    out1 = np.rollaxis(dout, 1).reshape(C, -1)
    out1 = np.rollaxis(out1, 1)

    dx, dgamma, dbeta = batchnorm_backward(out1, cache)
